...
import { ReportFiltersDto, InventoryReportFiltersDto } from './dto/report-filters.dto';
...
export class ReportsController {
  ...
  async getInventoryReport(
    @Query() filters: InventoryReportFiltersDto,
    @Res() res: Response,
  ) {
    const { format } = filters;

    if (format === 'excel') {
      const buffer = await this.reportsService.exportInventoryToExcel();
      res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    ...
    if (format === 'pdf') {
      const buffer = await this.reportsService.exportInventoryToPdf();
      res.setHeader('Content-Type', 'application/pdf');
